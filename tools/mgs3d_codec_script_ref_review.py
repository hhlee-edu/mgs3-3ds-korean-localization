#!/usr/bin/env python3
"""Cross-check the codec master against the script reference script, in one review file.

script reference is the only material in the project that carries a **speaker** for codec
...
* script reference merges what the game splits. `p1 s74` holds both `gcx 15 res 14` and
  `res 15`, so a master line is usually a *substring* of a script reference line, not
...
through untouched; nothing already confirmed is downgraded here.
    ...
    python tools/mgs3d_codec_script_ref_review.py
"""
...
SHINS = ROOT / "translation/20_matching/script_ref/script_ref_mgs3_classified.csv"
...
OUT = ROOT / "translation/10_master/review/codec-script_ref-review.csv"
...
def main() -> int:
    ...
    # script reference index: normalised text -> entries, plus a haystack for containment
    sh_entries = []
    for r in shins:
        sh_entries.append({
    ...
    def match_row(korean):
        """Return (entry, kind) for the best script reference containment match."""
        n = kstrip(korean)
        if not n:
            return None, ""
    ...
    # First pass: which GCX does script reference actually cover, and how strongly.
    ...
    for r in master:
        ...
        if not has_hangul(korean):
            ...
            # script reference covers only 414 codec lines against ~9,500 master rows, so
            # "no match" is usually just absence of a reference, not a defect. Only
            # call it a mismatch when script reference clearly covers the same beat.
            status, reason = ("NO_SCRIPT_REF_REF",
                              "script reference has no line for this dialogue")
        elif (gcx, res) in confirmed:
            status, reason = "SCRIPT_REF_MATCH", "confirmed anchor (preserved)"
        elif short:
        ...
        # Divergence is only meaningful inside a GCX script reference demonstrably covers.
        # script reference carries no English, so "these texts differ" is not by itself
        ...
        if (status == "NO_SCRIPT_REF_REF" and has_hangul(korean) and not short
                and not info_card):
            if anchored_gcx.get(gcx, 0) >= 2:
                status_out = "SCRIPT_REF_MISMATCH"
                reason = ("GCX has %d script reference matches but this line matches none"
                          % anchored_gcx[gcx])
        ...
        rows.append({
            ...
            "script_ref_speaker": entry["speaker"] if entry else "",
            "script_ref_korean": entry["text"] if entry else "",
            "script_ref_page_seq": ("%s:%s" % (entry["page"], entry["seq"])) if entry else "",
            "match_kind": kind,
    ...
    # script reference lines nothing in the master matched -- candidate exposed dialogue
    matched_norms = {kstrip(r["script_ref_korean"]) for r in rows if r["script_ref_korean"]}
    for e in sh_entries:
        if e["norm"] in matched_norms:
            continue
        rows.append({
            "status": "UNMATCHED_SCRIPT_REF", "gcx": "", "resource": "", "occurrences": "",
            "accept": "", "english": "", "korean_current": "", "recovered": "", "source": "",
            "script_ref_speaker": e["speaker"], "script_ref_korean": e["text"],
            "script_ref_page_seq": "%s:%s" % (e["page"], e["seq"]), "match_kind": "",
            "unencodable_glyphs": "", "ctx_prev_english": "", "ctx_next_english": "",
            "review_reason": "script reference codec line with no master counterpart",
        })
        stats["UNMATCHED_SCRIPT_REF"] += 1

    order = {"RECOVERED": 0, "SCRIPT_REF_MISMATCH": 1, "AMBIGUOUS": 2, "ENGLISH_REMAINS": 3,
             "UNMATCHED_SCRIPT_REF": 4, "SCRIPT_REF_MATCH": 5, "NO_SCRIPT_REF_REF": 6}
    ...
    print("script_ref codec lines            : %d" % len(sh_entries))
