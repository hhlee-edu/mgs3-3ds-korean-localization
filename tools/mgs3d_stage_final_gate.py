...
def main():
    ...
    allowed={(r['stage'],int(r['record']),int(r['resource'])) for r in locs if r['language']=='english' and r['raw_hex'] in ready}
    ...
    for src in files:
        ...
        for ri,(brec,arec) in enumerate(zip(br,ar)):
            ...
            for j,(old,new) in enumerate(zip(bs,rs)):
                ...
                if key not in allowed:errors.append(f'unexpected/non-EN change {key}')
