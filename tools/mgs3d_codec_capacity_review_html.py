...
def main() -> int:
    ...
    units = sorted(units, key=lambda unit: (int(unit["gcx"]), int(unit["resource"])))
...
let lastGcx=null;
$('#cards').innerHTML=groups.map((g,p)=>{const loc=g.indices.map(i=>data.physical[i]),limits=loc.map(r=>state[r.gcx].recommended),limit=Math.min(...limits),h=hangul(g.text).size,bytes=encoded(g.text),old=Math.min(...loc.map(r=>r.original_size)),gcx=loc[0].gcx,s=state[gcx];let head='';if(gcx!==lastGcx){lastGcx=gcx;head=`<div class="gcxhead">GCX ${gcx} · <span class="${s.remaining>=0?'ok':'bad'}">${s.remaining>=0?'여유':'초과'} ${Math.abs(s.remaining)}B</span> · ${s.rows}개</div>`}return head+`<article class="card ${p===current?'current':''} ${g.enabled?'':'off'}" data-p="${p}"><label><input type="checkbox" data-e="${g.id}" ${g.enabled?'checked':''}> 적용</label> <span class="meta">#${g.id} · ${loc.map(r=>`GCX ${r.gcx}/${r.resource}`).join(', ')}</span><div class="english">${esc(g.english)}</div><div class="meta">검수 원문</div><div class="full">${esc(g.full)}</div><textarea data-t="${g.id}">${esc(g.text)}</textarea><div class="metrics"><span>문장 ${bytes}/${old}B</span><span>고유 한글 ${h}자</span><span>권장 GCX 전체 ≤ ${limit}자</span><span class="${g.decision==='confirmed'?'ok':''}">${g.decision==='confirmed'?'확인됨':''}</span></div></article>`}).join('');
}
